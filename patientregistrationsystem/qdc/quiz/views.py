...
from models import Patient, SocialDemographicData, SocialHistoryData, \
    ...
    GenderOption, AmountCigarettesOption, AlcoholFrequencyOption, \
    AlcoholPeriodOption

from forms import PatientForm, SocialDemographicDataForm, \
    SocialHistoryDataForm, UserForm, UserFormUpdate
...
def patient_create(request, template_name="quiz/register.html"):
    ...
    if request.method == "POST":
        ...
        if patient_form.is_valid():
            ...
            homonym_message = ""
            homonym_list = Patient.objects.filter(
                name_txt=request.POST['name_txt']).exclude(removed=True)
            if homonym_list:
            ...
            if social_demographic_form.is_valid():
                ...
                new_social_demographic_data = social_demographic_form.save(
                    commit=False)
                ...
                if (new_social_demographic_data.tv_opt is not None and
                    ...
                    new_social_demographic_data.social_class_opt = new_social_demographic_data.calculate_social_class(
                        tv=request.POST['tv_opt'],
                        radio=request.POST['radio_opt'],
                        banheiro=request.POST['bath_opt'],
                        automovel=request.POST['automobile_opt'],
                        empregada=request.POST['house_maid_opt'],
                        maquina=request.POST['wash_machine_opt'],
                        dvd=request.POST['dvd_opt'],
                        geladeira=request.POST['refrigerator_opt'],
                        freezer=request.POST['freezer_opt'],
                        escolaridade=request.POST['schooling_opt'])
                else:
                    ...
                    if (new_social_demographic_data.tv_opt is not None or
                        ...
                        messages.warning(request,
                                         'Classe Social não calculada, pois os campos necessários '
                                         'para o cálculo não foram preenchidos.')

                new_social_demographic_data.save()

            if social_history_form.is_valid():
                new_social_history_data = social_history_form.save(
                    commit=False)
            ...
            return HttpResponseRedirect(
                redirect_url + "?currentTab=" + str(current_tab))

        else:

            if request.POST['cpf_id']:
                patient_found = Patient.objects.filter(
                    cpf_id=request.POST['cpf_id'])

                if patient_found:

                    if patient_found[0].removed:
                        patient_form.errors['cpf_id'][
                            0] = "Já existe paciente removido com este CPF."
                    else:
                        patient_form.errors['cpf_id'][
                            0] = "Já existe paciente cadastrado com este CPF."
    ...
    context = {
        ...
        'flesh_tone_options': flesh_tone_options,
        'marital_status_options': marital_status_options,
        'schooling_options': schooling_options,
...
def patient_update(request, patient_id, template_name="quiz/register.html"):
    ...
    if p and not p.removed:
        ...
        # # --------------------------------------
        try:
            p_social_demo = SocialDemographicData.objects.get(
                id_patient_id=patient_id)
            social_demographic_form = SocialDemographicDataForm(
        ...
        # # --------------------------------------
        try:
            p_social_hist = SocialHistoryData.objects.get(
                id_patient_id=patient_id)
            social_history_form = SocialHistoryDataForm(request.POST or None,
                                                        instance=p_social_hist)
        ...
        if request.method == "POST":
            ...
            if patient_form.is_valid():
                ...
                if request.POST['cpf_id'] and Patient.objects.filter(
                        cpf_id=request.POST['cpf_id']):
                    patient_form.errors['cpf_id'][
                        0] = "Já existe paciente cadastrado com este CPF."
...
def patient(request, patient_id, template_name="quiz/register.html"):
    ...
    if p and not p.removed:
        ...
        for form in {patient_form, social_demographic_form,
                     social_history_form}:
        ...
        patient_form.fields['state_txt'].widget = SelectBoxStateDisabled(
            attrs={'data-country': 'id_country_state_address',
                   'id': 'id_chosen_state', 'disabled': 'true'})
        patient_form.fields[
            'citizenship_txt'].widget = SelectBoxCountriesDisabled(
        ...
        if search_text:
            if re.match('[a-zA-Z ]+', search_text):
                patient_list = Patient.objects.filter(
                    name_txt__icontains=search_text).exclude(removed=True)
            else:
                patient_list = Patient.objects.filter(
                    cpf_id__icontains=search_text).exclude(removed=True)
        else:
            patient_list = ''
    else:
        search_text = ''

    return render_to_response('quiz/ajax_search.html',
...
def user_update(request, user_id, template_name="quiz/register_users.html"):
    form = UserFormUpdate(request.POST or None,
