...
def concatenate_respecting_max_length(first_string, middle_string, second_string, max_length):
    '''
    fazer string caber no máximo tamanho

    input: first_string, second_string, max_length, middle_string
    '''

    if len(first_string + middle_string + second_string) <= max_length:
        return first_string + middle_string + second_string

    if len(first_string) > len(second_string):
        bigger = first_string
        smaller = second_string
        is_first_bigger = True
    else:
        bigger = second_string
        smaller = first_string
        is_first_bigger = False

    #diff => tamanho máximo da string final, sem a string do meio
    available_space = max_length - len(middle_string)

    # tratar o menor -> assumir que o máximo tamanho será 1/2 string
    space_used = min(len(smaller), floor(available_space / 2))
    smaller = smaller[:space_used]

    # tratar o maior
    space_left = available_space - space_used
    bigger = bigger[:space_left]

    # montar string final como first_string + second_string
    if is_first_bigger:
        return bigger + middle_string + smaller
    else:
        return smaller + middle_string + bigger
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for experiment in orm.Experiment.objects.all():
            ...
            group = orm.Group(experiment=experiment,
                              title='Default',
                              description='Automatic generated by migration')
            group.save()

            # Migrate subjects
            for subject_of_experiment in orm.SubjectOfExperiment.objects.filter(experiment=experiment):
                subject_of_group = orm.SubjectOfGroup(subject=subject_of_experiment.subject,
                                                      group=group,
                                                      consent_form=subject_of_experiment.consent_form)
                subject_of_group.save()

                for questionnaire_response in orm.QuestionnaireResponse.objects.filter(
                        subject_of_experiment=subject_of_experiment):
                    questionnaire_response.subject_of_group = subject_of_group
                    questionnaire_response.save()

                for historical_questionnaire_response in orm.HistoricalQuestionnaireResponse.objects.filter(
                        subject_of_experiment_id=subject_of_experiment.id):
                    historical_questionnaire_response.subject_of_group = subject_of_group
                    historical_questionnaire_response.save()

            # Migrate questionnaires
            for questionnaire_configuration in orm.QuestionnaireConfiguration.objects.filter(
                    experiment=experiment):
                questionnaire_configuration.group = group
                questionnaire_configuration.save()

            for historical_questionnaire_configuration in orm.HistoricalQuestionnaireConfiguration.objects.filter(
                    experiment_id=experiment.id):
                historical_questionnaire_configuration.group = group
                historical_questionnaire_configuration.save()

    def backwards(self, orm):
        experiments_to_be_deleted = []

        for group in orm.Group.objects.all():
            experiment = group.experiment
            experiments_to_be_deleted.append(experiment)

            # Migrate basic information
            title = "Group " + group.title + " of experiment " + experiment.title

            if len(title) > 50:
                title = concatenate_respecting_max_length(experiment.title, " - ", group.title, 50)

            description = concatenate_respecting_max_length(experiment.description, linesep, group.description, 150)

            new_experiment = orm.Experiment(title=title,
                                            description=description)
            new_experiment.save()

            # Migrate subjects
            for subject_of_group in orm.SubjectOfGroup.objects.filter(group=group):
                subject_of_experiment = orm.SubjectOfExperiment(subject=subject_of_group.subject,
                                                                experiment=new_experiment,
                                                                consent_form=subject_of_group.consent_form)
                subject_of_experiment.save()

                # This is needed to allow deletion of the group while deleting the old experiment (cascade)
                # subject_of_group.group = None
                # subject_of_group.save()
                ...
                for historical_questionnaire_response in orm.HistoricalQuestionnaireResponse.objects.filter(
                    ...
                    historical_questionnaire_response.subject_of_experiment = subject_of_experiment
                    historical_questionnaire_response.save()

            # Migrate questionnaires
            for questionnaire_configuration in orm.QuestionnaireConfiguration.objects.filter(
                    group=group):
                questionnaire_configuration.experiment = new_experiment
                # This is needed to allow deletion of the group while deleting the old experiment (cascade)
                questionnaire_configuration.group_id = None
                questionnaire_configuration.save()

            for historical_questionnaire_configuration in orm.HistoricalQuestionnaireConfiguration.objects.filter(
                    group_id=group.id):
                historical_questionnaire_configuration.experiment = new_experiment
                historical_questionnaire_configuration.save()

        for experiment in experiments_to_be_deleted:
            experiment.delete()
